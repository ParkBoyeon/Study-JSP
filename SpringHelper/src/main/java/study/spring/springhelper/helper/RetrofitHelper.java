...
public class RetrofitHelper {
	...
	public Retrofit getRetrofit(String baseUrl) {
		...
		// 통신 객체에 Log 기록 개체를 연결한다.
		HttpLoggingInterceptor logging = new HttpLoggingInterceptor(new MyLogger());
		logging.level(Level.BODY);
		httpClientBuiler.addInterceptor(logging);
		
		// MyInterceptor 클래스의 객체를 통신 객체에 연결한다.
		httpClientBuiler.addNetworkInterceptor(new MyInterceptor());
		
		// Builder를 통해 통신 객체 생성
		OkHttpClient httpClient = httpClientBuiler.build();
